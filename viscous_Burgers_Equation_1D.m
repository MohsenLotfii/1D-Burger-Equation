...
%         u(i)=un(i)-(CN/2)*((un(i)).^2-(un(i-1)).^2)+(vis*dt/dx^2)*(un(i-1)-2*un(i)+un(i+1));
        
% SOU:

%         u(i)=un(i)-(CN/2)*((3/2*un(i)-1/2*un(i-1)).^2-((3/2)*un(i-1)-(1/2)*un(i-2)).^2)+ (vis*dt/2*(dx^2))*(-un(i)+2*un(i-1)-un(i-2));
        
% QUICK: 

        u(i)=un(i)- (CN/2)*(((3/8)*un(i+1)+(3/4)*un(i)-(1/8)*un(i-1)).^2-((3/8)*un(i)+(3/4)*un(i-1)-(1/8)*un(i-2)).^2) + (vis*dt/8*(dx^2)).*(-un(i-2)+5*un(i-1)-7*un(i)+3*un(i+1));
...
% axis([0 20 0 1])
% title({['1-D viscous Burger Equation'];['time(\itt) = ',num2str(dt*it)]})
% xlabel('Spatial co-ordinate (x) \rightarrow')
% ylabel('Transport property profile (u) \rightarrow')
% drawnow;
% refreshdata(h)
